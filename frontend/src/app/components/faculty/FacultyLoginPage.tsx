...
export function FacultyLoginPage({ onBack, onLoginSuccess, onNewUser }: FacultyLoginPageProps) {
  ...
  const validate = () => { const e: {[k:string]:string} = {}; if (!formData.username.trim()) e.username = 'Username required'; if (!formData.password) e.password = 'Password required'; else if (formData.password.length < 6) e.password = 'Min 6 characters'; if (!captchaOk) e.captcha = 'Complete the CAPTCHA'; setErrors(e); return Object.keys(e).length === 0 }
  const handleSubmit = async (ev: React.FormEvent) => { ev.preventDefault(); if (!validate()) return; setIsLoading(true); setTimeout(() => { setIsLoading(false); if (formData.username.trim() && formData.password.length >= 6) { localStorage.setItem('currentUser', JSON.stringify({ username: formData.username, name: 'Faculty User', role: 'faculty' })); onLoginSuccess() } else { setErrors({ general: 'Invalid credentials.' }); recaptchaRef.current?.reset(); setCaptchaOk(false) } }, 1200) }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
      ...
      <motion.div className="w-full max-w-md relative z-10" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <motion.button onClick={onBack} className="flex items-center gap-2 text-sm text-slate-500 hover:text-emerald-600 mb-6 transition-colors" whileHover={{ x: -3 }}><ArrowLeft className="w-4 h-4" /> Back</motion.button>
        <div className="bg-white rounded-2xl border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
          <div className="bg-gradient-to-r from-emerald-600 to-teal-500 px-8 py-10 text-center relative overflow-hidden">
            <div className="absolute inset-0 opacity-10"><svg width="100%" height="100%"><defs><pattern id="df" width="20" height="20" patternUnits="userSpaceOnUse"><circle cx="2" cy="2" r="1" fill="white"/></pattern></defs><rect width="100%" height="100%" fill="url(#df)"/></svg></div>
            <motion.div className="relative mx-auto w-16 h-16 rounded-2xl bg-white/20 flex items-center justify-center mb-5 backdrop-blur-sm" initial={{ scale: 0, rotate: -180 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: 'spring', stiffness: 200, delay: 0.2 }}><BookOpen className="w-8 h-8 text-white" /></motion.div>
          ...
          <div className="p-6 sm:p-8">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}><Button type="submit" className="w-full h-11 bg-gradient-to-r from-emerald-600 to-teal-500 text-white hover:from-emerald-700 hover:to-teal-600 rounded-xl font-semibold shadow-lg shadow-emerald-500/25" disabled={isLoading}>{isLoading ? <div className="flex items-center gap-2"><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Signing In...</div> : 'Sign In'}</Button></motion.div>
